...
public class Access_BD_Employe {
	 ...
	 */
	public ArrayList<Employe> getEmployes(int idAg, int idNumEmp, String debutNom, String debutPrenom)
		...
		try {
			...
			while (rs.next()) {				
				...
				alResult.add(
						new Employe(idEmployeTrouve, nom, prenom, droitsAccess, loginTROUVE, motPasseTROUVE, idAgEmploye));
			}
			rs.close();
			pst.close();
